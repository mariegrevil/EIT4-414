...
/*
ABOUT TIMERS
following is Arduinos impl found in ...
"arduino install"/hardware/arduino/avr/cores/arduino/wiring.c
...
{
  long msc = ticks * k_tick_size; // now in milliseconds
  if (msc <= 0) // should never happen
    return;
  while (msc--) { // not 100% accurate.
    _delay_ms(1);
  }
}
