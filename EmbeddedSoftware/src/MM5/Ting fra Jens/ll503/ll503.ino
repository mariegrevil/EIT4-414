...
struct k_t *timerTick1,*timerTick2;
void t1()
{
  k_set_sem_timer(timerTick1,50);
  while (1) {
    k_wait(timerTick1,0);
    k_eat_msec_time(10); // simulate running some code
   }
}
 
void t2()
{
  k_set_sem_timer(timerTick2,50);
  while (1) {
    k_wait(timerTick2,0);
    k_eat_msec_time(60); // simulate running some code
    k_sle
   }
}
